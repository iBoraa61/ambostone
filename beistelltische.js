(() => {
  ...
  document.addEventListener('click', (e) => { const card = e.target.closest('.wb-gridView .card'); if (!card) return; const _t = safeStr(card.dataset.title); if (!_t) return; const url = 'produkt.html?src=beistelltische.html&p=' + encodeURIComponent(_t); const a = document.createElement('a'); a.href=url; a.target='_blank'; a.rel='noopener'; document.body.appendChild(a); a.click(); document.body.removeChild(a); });
